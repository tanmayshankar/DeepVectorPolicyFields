...
class BPRCNN():
	...
	def construct_from_ext_state(self):
		...
		# self.from_angular_ext[w-1] = self.from_angular_belief[-1]
		# self.from_angular_ext[self.discrete_theta+w] = self.from_angular_belief[0]
...
def main(args):    
	...
	for i in range(8):	
		bprcnn.load_trajectory(traj[i],((-1)**i)*orient[i])
		bprcnn.train_BPRCNN()
