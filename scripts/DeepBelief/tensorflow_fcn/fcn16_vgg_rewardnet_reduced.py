...
class FCN16VGG:
	def __init__(self, vgg16_npy_path=None):
		...
		if not os.path.isfile(vgg16_npy_path):
			logging.error(("File '%s' not found. Download it from " "https://dl.dropboxusercontent.com/u/" "50333326/vgg16.npy"), vgg16_npy_path)
			sys.exit(1)
	...
	def build(self, rgb, train=True, num_classes=4*18, random_init_fc8=False, debug=False):
		...
		self.fuse_pool4 = tf.add(self.upscore2, self.score_pool4)
 		self.upscore32 = self._upscore_layer(self.fuse_pool4, shape=tf.shape(self.pool3), num_classes=num_classes, debug=debug, name='upscore32', ksize=10, stride=2)
		# self.upscore32 = self._upscore_layer(self.fuse_pool4, shape=[1,48,46,72], num_classes=num_classes, debug=debug, name='upscore32', ksize=10, stride=2) 		
